    ...
    if (datePattern.test(potentialDate)) {
      ...
      if (amountStr) {
        ...
        const transaction = {
          ...
          amount: amount,
          category: await categorizeTransaction(cleanDescription, amount)
        };
...
async function categorizeTransaction(description, amount = null) {
  ...
  // 2. Custom user-specific rules (highest priority)
  const desc = description.toLowerCase();
  
  // Public Partnerships LLC -> Salary Income
  ...
  // --- END IMPROVED TRANSFER DETECTION ---
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <button class="btn" onclick="recategorizeTransactions()">🔄 Recategorize All Transactions</button>
    ...
    <script>
        ...
        async function recategorizeTransactions() {
            if (confirm('This will recategorize all transactions using improved logic. This may take a moment. Continue?')) {
                try {
                    showStatus('Recategorizing transactions... Please wait.', 'info');
                    const response = await fetch(\`\${API_BASE}/admin/recategorize-transactions\`, {
                        method: 'POST',
                        headers: { 
                    ...
                    if (response.ok) {
                        showStatus(\`\${result.message}. Updated \${result.updatedCount} transactions.\`);
                    } else {
                        showStatus('Recategorization failed: ' + result.error, 'error');
                    }
                } catch (error) {
                    showStatus('Recategorization error: ' + error.message, 'error');
                }
            }
        }
...
// Admin endpoint to recategorize all transactions (requires authentication)
app.post('/api/admin/recategorize-transactions', authenticateToken, async (req, res) => {
  try {
    console.log('Starting transaction recategorization...');
    
    // Get all transactions for the user
    const transactions = await new Promise((resolve, reject) => {
      db.all(`SELECT t.* FROM transactions t 
              LEFT JOIN accounts a ON t.account_name = a.name 
              WHERE (a.user_id = ? OR a.user_id IS NULL)`, [req.userId], (err, rows) => {
        if (err) reject(err);
        else resolve(rows);
      });
    });
    ...
    for (const transaction of transactions) {
      try {
        const newCategory = await categorizeTransaction(transaction.description, transaction.amount);
        
        // Only update if category changed
        if (newCategory !== transaction.category) {
          await new Promise((resolve, reject) => {
            db.run('UPDATE transactions SET category = ? WHERE id = ?', 
              [newCategory, transaction.id], (err) => {
                if (err) reject(err);
                else {
    ...
    res.json({
      message: `Successfully recategorized ${updatedCount} out of ${transactions.length} transactions`,
      updatedCount,
      totalCount: transactions.length,
    ...
    res.status(500).json({ error: 'Failed to recategorize transactions: ' + error.message });
  }
});
    ...
    if (err) {
      ...
      db.all(`SELECT ROUND(SUM(amount), 2) as total_expenses FROM transactions 
        ...
        db.all(`SELECT ROUND(SUM(amount), 2) as total_income FROM transactions 
                ...
                AND category IN ('Income', 'Salary Income', 'Splitwise Settlement')`, [req.userId], (incomeErr, incomeRow) => {
          if (incomeErr) {
...
// Categories endpoint - now dynamic based on actual usage
app.get('/api/categories', authenticateToken, (req, res) => {
  // Get all unique categories from transactions, plus default categories
  ...
  db.all(
    ...
    (err, rows) => {
      ...
      const usedCategories = rows.map(row => row.category);
